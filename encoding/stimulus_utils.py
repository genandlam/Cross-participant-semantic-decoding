...
from encoding.textgrid import TextGrid

def load_textgrids(stimuli, grid_dir: str):
    grids = {}
    for stim in stimuli:
        grid_path = os.path.join(grid_dir, "%s.TextGrid" % stim)
        grids[stim] = TextGrid(open(grid_path).read())
    return grids

def load_simulated_trfiles(respdict, tr=2.0, start_time=10.0, pad=5):
    trdict = dict()
    for stim, resps in respdict.items():
        trf = TRFile(None, tr)
